...
class S3Service extends Service {
  ...
  async readObjectToString({ bucket, key }) {
    ...
    const stream = this.api.getObject({ Bucket: bucket, Key: key }).createReadStream();
  ...
  async deleteObject({ bucket, key }) {
    return this.api.deleteObject({ Bucket: bucket, Key: key }).promise();
  }
}
