---
to: ../packages/services/lib/<%= h.changeCase.paramCase(name) %>/<%= h.changeCase.paramCase(name) %>-service.js
---
const _ = require('lodash');
const Service = require('@aws-ee/services-container/lib/service');

const { runAndCatch } = require('../helpers/utils');
const createSchema = require('../schema/create-<%= h.changeCase.paramCase(name) %>');
const updateSchema = require('../schema/update-<%= h.changeCase.paramCase(name) %>');

const settingKeys = {
  tableName: 'dbTable<%= h.changeCase.ucFirst(h.changeCase.camel(h.inflection.pluralize(name))) %>',
};

class <%= h.changeCase.ucFirst(h.changeCase.camel(name)) %>Service extends Service {
  constructor() {
    super();
  ...
  async mustFind(requestContext, { id, fields = [] }) {
    ...
    if (!result) throw this.boom.notFound(`<%= h.changeCase.camel(name) %> with id "${id}" does not exist`, true);
    return result;
  }
  ...
  async create(requestContext, rawData) {
    ...
    const result = await runAndCatch(async () => {
      ...
      throw this.boom.badRequest(`<%= h.changeCase.camel(name) %> with id "${id}" already exists`, true);
    });
  ...
  async update(requestContext, rawData) {
    ...
    const result = await runAndCatch(async () => {
      ...
      // 1 - The <%= name.toLowerCase() %> does not exist
      ...
      if (existing) {
        throw this.boom.badRequest(`<%= h.changeCase.camel(name) %> information changed by "${(existing.updatedBy || {}).username}" just before your request is processed, please try again`, true);
      }
      throw this.boom.notFound(`<%= h.changeCase.camel(name) %> with id "${id}" does not exist`, true);
    });
...
module.exports = <%= h.changeCase.ucFirst(h.changeCase.camel(name)) %>Service;
